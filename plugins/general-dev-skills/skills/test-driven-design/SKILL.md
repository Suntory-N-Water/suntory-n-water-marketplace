...
テストファイル内のテストは以下の階層でグルーピングする。人間が後からテストを読んだときに、対象の振る舞い・シナリオを把握しやすくすることが目的。
...
describe("calculateDiscount", () => {
  describe("正常系", () => {
    describe("会員ランクによる割引", () => {
      test("ゴールド会員の場合20%割引が適用される", () => { ... })
      test("シルバー会員の場合10%割引が適用される", () => { ... })
    })
    describe("キャンペーン期間中", () => {
      test("対象商品に追加5%割引が適用される", () => { ... })
    })
  })
  describe("異常系", () => {
    describe("無効な入力値", () => {
      test("注文金額が0以下の場合エラーが発生する", () => { ... })
    })
  })
})
```

グルーピングの粒度はビジネスの文脈に合わせる。機械的にまとめるのではなく、「このグループを読めばどんな前提条件・シナリオか分かる」という単位を意識する。
